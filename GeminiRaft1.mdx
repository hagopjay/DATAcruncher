...
class VectorDB:
    def __init__(self, name: str, api_key=None):
        if api_key is None:
            api_key = os.getenv("GEMINI_API_KEY")  # Use GEMINI_API_KEY
    ...
    def _embed_and_store(self, texts: List[str], data: List[Dict[str, Any]]):
        ...
        with tqdm(total=len(texts), desc="Embedding chunks") as pbar:
            ...
            for i in range(0, len(texts), batch_size):
                ...
                response = genai.embed(model="models/embedding-001", texts=batch)
                batch_result = [embedding.values for embedding in response.embeddings]
                result.extend(batch_result)
                pbar.update(len(batch))
    ...
    def search(self, query: str, k: int = 20) -> List[Dict[str, Any]]:
        if query in self.query_cache:
            ...
            # Use Gemini for embedding the query
            response = genai.embed(model="models/embedding-001", texts=[query])
...
# ... (rest of your code - load_jsonl, evaluate_retrieval, retrieve_base, evaluate_db) ...
